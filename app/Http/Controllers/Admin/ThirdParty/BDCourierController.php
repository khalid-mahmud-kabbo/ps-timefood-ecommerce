<?php

namespace App\Http\Controllers\Admin\ThirdParty;

use App\Contracts\Repositories\BusinessSettingRepositoryInterface;
use App\Enums\ViewPaths\Admin\SocialMediaChat;
use App\Http\Controllers\BaseController;
use Devrabiul\ToastMagic\Facades\ToastMagic;
use Illuminate\Contracts\View\View;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;

class BDCourierController extends BaseController
{

    public function __construct(
        private readonly BusinessSettingRepositoryInterface $businessSettingRepo,
    )
    {
    }

    /**
     * @param Request|null $request
     * @param string|null $type
     * @return View Index function is the starting point of a controller
     * Index function is the starting point of a controller
     */
    public function index(Request|null $request, string $type = null): View
    {
        return $this->getView();
    }

    public function getView(): View
    {
        return view('admin-views.third-party.bd-courier.view');
    }

    public function update(Request $request, $service): RedirectResponse
    {
        if($service == 'bdcourier'){
            $value = json_encode(['status' => $request->get('status', 0), 'api_key' => $request['api_key'], 'api_url' => $request['api_url']]);
            $this->businessSettingRepo->updateOrInsert(type: 'bdcourier', value: $value);
        }else{
            ToastMagic::warning(translate($service . '_information_update_fail'));
            return back();
        }

        ToastMagic::success(translate($service . '_information_update_successfully'));
        return back();
    }
}
